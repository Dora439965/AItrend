...
// @AI_GENERATED: 热榜→新手推荐联动状态
let fitTargetRepoId = null; // 用户从热榜点击"适合我吗"时记录的目标项目 ID
// @AI_GENERATED: end
...
document.addEventListener("click", async (event) => {
  ...
  if (actionButton) {
    ...
    if (action === "fit") {
      // @AI_GENERATED: 记录目标项目并跳转到新手推荐模块
      fitTargetRepoId = actionButton.dataset.repo;
      setActiveSection("recommender");
      // 隐藏右侧"完成问卷后生成推荐"提示框
      const emptyReco = document.querySelector("#beginnerEmptyReco");
      if (emptyReco) emptyReco.classList.add("hidden");
      const recoAside = document.querySelector("#beginnerRecommendations");
      if (recoAside) recoAside.classList.add("hidden");
      // 在问卷顶部显示目标项目提示
      renderFitTargetHint();
      showToast("请完成画像问卷，系统将为你评估与该项目的匹配度");
      // @AI_GENERATED: end
    }
    // @AI_GENERATED
    if (action === "save") {
  ...
  if (beginnerNav) {
    ...
    if (direction === "next" && currentBeginnerStep === totalSteps - 1) {
      ...
      if (isQuestionnaireComplete()) {
        ...
        // @AI_GENERATED: 如果有来自热榜的目标项目，直接生成针对该项目的匹配报告
        if (fitTargetRepoId) {
          const targetRepo = repos.find((r) => String(r.id) === String(fitTargetRepoId));
          if (targetRepo) {
            showFitResult(userProfile, targetRepo);
            fitTargetRepoId = null;
            const hint = document.querySelector("#fitTargetHint");
            if (hint) hint.remove();
            // 恢复右侧推荐区域
            const recoAside = document.querySelector("#beginnerRecommendations");
            if (recoAside) recoAside.classList.remove("hidden");
            return;
          }
        }
        // @AI_GENERATED: end
  ...
  if (drawerAction) {
    ...
    if (action === "open-github" || action === "open-repo") {
      const url = drawerAction.dataset.url;
      if (url) {
        window.open(url, "_blank", "noopener");
      } else {
    ...
    // @AI_GENERATED: 匹配报告中的"生成完整上手文档"按钮，使用新手推荐模块的入门指南格式
    if (action === "fit-generate-doc") {
      const repoId = drawerAction.dataset.repo;
      const repo = repos.find((r) => String(r.id) === String(repoId));
      if (repo) {
        const userProfile = buildUserProfile();
        const project = generateFitReport(userProfile, repo).project;
        openBeginnerDocDrawer(project, userProfile);
      } else {
        showToast("未找到项目数据");
      }
...
// @AI_GENERATED: 热榜「适合我吗」联动 — 显示目标项目提示 + 生成匹配报告
function renderFitTargetHint() {
  const container = document.querySelector("#beginnerWizardContainer");
  if (!container || !fitTargetRepoId) return;
  const repo = repos.find((r) => String(r.id) === String(fitTargetRepoId));
  if (!repo) return;

  // 在问卷顶部插入目标提示卡片
  const hintId = "fitTargetHint";
  let hint = document.querySelector(`#${hintId}`);
  if (!hint) {
    hint = document.createElement("div");
    hint.id = hintId;
    hint.className = "fit-target-hint";
    container.parentElement.insertBefore(hint, container);
  }
  hint.innerHTML = `
    <div class="fit-target-card">
      <p class="eyebrow">正在评估你与以下项目的匹配度</p>
      <h4>${repo.owner}/${repo.name}</h4>
      <p>${repoDescription(repo)}</p>
      <button class="small-button" id="cancelFitTarget">取消</button>
    </div>
  `;
  hint.querySelector("#cancelFitTarget").addEventListener("click", () => {
    fitTargetRepoId = null;
    hint.remove();
    // 恢复右侧推荐区域显示
    const recoAside = document.querySelector("#beginnerRecommendations");
...
function generateFitReport(userProfile, repo) {
  // 将热榜 repo 转为 beginner project 格式，以便复用 computeMatchScore
  const project = {
    id: repo.id,
    name: repo.name,
    owner: repo.owner,
    ...
    hasChineseDocs: !!(repo.descriptionZh),
    hasExamplesFolder: false,
    lastUpdated: repo.pushedAt || new Date().toISOString(),
    topics: repo.tags || [],
    forks: repo.forks || 0,
    openIssues: repo.openIssues || 0,
  };

  const scoreResult = computeMatchScore(userProfile, project);
  const reason = generateMatchReasonText(userProfile, project, scoreResult.breakdown);

  return { project, ...scoreResult, reason };
}

function showFitResult(userProfile, repo) {
  const result = generateFitReport(userProfile, repo);
  const matchLevel = result.total >= 70 ? "高度匹配" : result.total >= 45 ? "中等匹配" : "匹配度较低";
  const matchColor = result.total >= 70 ? "hot" : result.total >= 45 ? "up" : "";

  const body = `
    <section class="drawer-section">
      <h3>匹配度评估</h3>
      <div class="metric-row">
        <span class="metric ${matchColor}" style="font-size:1.3em;">匹配度 ${result.total}%</span>
        <span class="metric">${matchLevel}</span>
      </div>
    </section>
    <section class="drawer-section">
      <h3>为什么${result.total >= 45 ? "适合" : "不太适合"}你</h3>
      <div class="why-fit-content">${result.reason.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\n/g, "<br>")}</div>
    </section>
    <section class="drawer-section">
      <h3>项目基本信息</h3>
      <p>${repoDescription(repo)}</p>
      <div class="metric-row">
        <span class="metric">难度 ${repo.difficulty || "进阶"}</span>
        <span class="metric">${repo.language}</span>
        <span class="metric">${formatNumber(repo.stars)} stars</span>
      </div>
    </section>
    <section class="drawer-section">
      <h3>入门建议</h3>
      <ol>
        <li><strong>克隆仓库</strong> — <code>git clone ${repo.url || `https://github.com/${repo.owner}/${repo.name}`}</code></li>
        <li><strong>阅读 README</strong> — 重点看 Quick Start 和 Prerequisites 部分</li>
        <li><strong>安装依赖</strong> — 根据项目语言(${repo.language})执行对应的依赖安装命令</li>
        <li><strong>运行示例</strong> — 找到 examples 或 demo 目录，先跑通最简单的示例</li>
        <li><strong>做第一个修改</strong> — 改一个提示词、换一个模型名、或加一个输入字段</li>
      </ol>
    </section>
    <section class="drawer-section">
      <h3>需要的前置知识</h3>
      <div class="tag-row">${(repo.knowledge || []).map((k) => `<span class="tag">${k}</span>`).join("")}</div>
    </section>
    <section class="drawer-section">
      <h3>风险提示</h3>
      <p>${repo.risk || "请结合 README 和实际运行情况确认。"}</p>
    </section>
  `;

  openDrawer("匹配报告", `${repo.owner}/${repo.name}`, body, `
    <button class="primary-button" data-drawer-action="open-repo" data-url="${repo.url || `https://github.com/${repo.owner}/${repo.name}`}">打开 GitHub</button>
    <button class="secondary-button" data-drawer-action="fit-generate-doc" data-repo="${repo.id}">生成完整上手文档</button>
  `);
}
// @AI_GENERATED: end
